...
- Visual Refresh track: DESIGN.md uses YAML front-matter with color tokens; design tokens map to tkinter colors (e.g., `--color-background` → `#050505`, `--color-danger` → `#FF2D55`). Tooltip background updated from `#333333` to `#0D0D0D` (surface), placeholder text from "gray" to `#555555` (text-muted). Badge colors in templates use semantic palette (Built-in: `#B366FF` ultraviolet, Custom: `#26FF8C` krypton).